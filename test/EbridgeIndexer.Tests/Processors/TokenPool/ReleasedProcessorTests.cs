...
namespace EbridgeIndexer.Processors.TokenPool;

public class ReleasedProcessorTests: EbridgeIndexerTestBase
{
    private readonly ReleasedProcessor _releasedProcessor;
    private readonly IReadOnlyRepository<PoolLiquidityRecordIndex> _poolRepository;

    private readonly IObjectMapper _objectMapper;

    public ReleasedProcessorTests()
    {
        _releasedProcessor = GetRequiredService<ReleasedProcessor>();
    ...
    {
        var logEvent = new Released
        {
            TargetTokenSymbol = "ELF",
            Amount = 100
        };
        var logEventContext = GenerateLogEventContext(logEvent);
        await _releasedProcessor.ProcessAsync(logEvent, logEventContext);
        
        var logEvent1 = new Released
        {
            TargetTokenSymbol = "ELF",
            Amount = 50
        };
        ...
        await _releasedProcessor.ProcessAsync(logEvent1, logEventContext1);
